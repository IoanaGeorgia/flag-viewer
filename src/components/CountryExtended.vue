...
export default {
    ...
    created(){
        console.log(this.countryInfo, 'gg')
    },
    computed:{
      countryInfo(){
    ...
    return population.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.')
      ...
      goToBordering(border){
        let borderingCountryInfo = json.filter((entry) => entry.alpha3Code === border);
        this.countryInfoBorder = borderingCountryInfo[0]
        this.clickedBorder = true
      },
      convertBorder(border){
        let filteredBorderObjects = json.filter((entry) => entry.alpha3Code === border);
...
<template>
  ...
  <div class="detailsInnerWrapper">
    ...
    <div class="detailsInfo">
      ...
      <span class='countryInfoSpan'>{{ populationFormatter(countryInfo.population) }}</span>
    </p> 
      ...
      <span class='countryInfoSpan'><span v-for='level in countryInfo.topLevelDomain'>{{ level }}</span></span>
    </p> 
      <p class="countryInfo">Currencies: 
      <span class='countryInfoSpan'>{{ countryInfo.currencies[0].name}}</span>
    </p> 
      <p class="countryInfo">Languages: 
      <span class='countryInfoSpan'>{{ languageFormatter(countryInfo.languages) }}</span>
    </p> 
        </div>
